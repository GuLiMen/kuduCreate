...
//        kuduMaster = "dev02";
        kuduTable = "ODS_CREATE_DATE";         //指定表名
...
//        删除字段，但是主键不能删除
        ...
        try {
            kuduClient.alterTable(kuduTable, new AlterTableOptions().dropColumn("WORKSHOP_CODE"));
            kuduClient.alterTable(kuduTable, new AlterTableOptions().dropColumn("MACHINE_NO"));
            kuduClient.alterTable(kuduTable, new AlterTableOptions().dropColumn("FACTORY_NAME"));
            kuduClient.alterTable(kuduTable, new AlterTableOptions().dropColumn("DEVICE_M_ID"));
        } catch (KuduException e) {
            e.printStackTrace();
        }
    }
