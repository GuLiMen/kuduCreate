    ...
    public void renameTable(){
        try {
            kuduClient.alterTable(kuduTable,
                    new AlterTableOptions().renameTable("MESC_I_MATERIAL"));
        } catch (KuduException e) {
            e.printStackTrace();
        }
    }
    ...
    public void creatTable(){
        ...
        try {
            ...
            if (!kuduClient.tableExists(kuduTable)){
                ...
                columnSchemas.add(new ColumnSchema.ColumnSchemaBuilder("PART_CODE",Type.STRING).key(true).build());
                columnSchemas.add(new ColumnSchema.ColumnSchemaBuilder("PART_NAME",Type.STRING).nullable(true).build());
                ...
                columnSchemas.add(new ColumnSchema.ColumnSchemaBuilder("TYPE2",Type.STRING).nullable(true).build());
                columnSchemas.add(new ColumnSchema.ColumnSchemaBuilder("PIXEL",Type.STRING).nullable(true).build());
                columnSchemas.add(new ColumnSchema.ColumnSchemaBuilder("PROCESS_SPEC",Type.STRING).nullable(true).build());
