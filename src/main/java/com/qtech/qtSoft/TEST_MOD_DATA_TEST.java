...
public class TEST_MOD_DATA_TEST {
    ...
    public void renameTable(){
        try {
            kuduClient.alterTable(kuduTable,
                    new AlterTableOptions().renameTable("MESC_TEST_MOD_DATA"));
        } catch (KuduException e) {
            e.printStackTrace();
        }
    }
    ...
    public void creatTable(){
        ...
        try {
            ...
            if (!kuduClient.tableExists(kuduTable)){
                ...
                columnSchemas.add(new ColumnSchema.ColumnSchemaBuilder("TESTTIME",Type.STRING).key(true).defaultValue(ft.format(date)).build());
